package com.jiekebo.ServiceDiscovery;
...
public class TestServer {
	...
	public static void main(String[] args) {
		...
		/*
		 * In an ideal server code, you should first do your own 'query' at this point
		 * to see if the service instance name already exists on the local network. If
		 * it does, you'll need to abort/exit, or retry with a modified service instance
		 * name, possibly by using a suffix such as "(2)" on the end. For this
		 * demo, we'll assume we're the only one, and keep going.
		 */
		
		
		/*
		 * We're okay so far. Now set up a responder and give it the descriptor
		 * we want to publish. Also to attempt graceful handling of ctrl-C, add
		 * a shutdown hook which tries to alert the network we're no longer providing
		 * this service (in case someone's watching a dynamic display of available
		 * servers). Finally start the responder (which works in its own thread).
		 */
		...
		/*
		 * Now we'll just hang out for a while until you stop the program.
		 * Any incoming connections will just have this machine's current
